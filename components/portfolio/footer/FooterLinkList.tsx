...
export const FooterLinkList = ({ links }: { links: readonly FooterLink[] }) => (
  <ul className="mt-7 space-y-4 text-[15px] text-[#bec9c4] max-desktop:mt-5 max-desktop:space-y-3">
    {links.map((item) => (
      <li key={item.label}>
        {item.href ? (
          <Link
          ...
          >
            ...
            <span
              className="text-accent opacity-0 transition-opacity group-hover:opacity-100 max-desktop:opacity-100"
              aria-hidden="true"
            >
