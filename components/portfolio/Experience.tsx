  ...
  return (
    ...
    >
      <div className="mb-16 grid grid-cols-[1fr_1.7fr_1fr] items-end gap-10 max-[1000px]:grid-cols-[1fr_2fr] max-[650px]:grid-cols-1 max-[650px]:gap-[22px]">
        <div>
          <span className="mb-2.5 block font-mono text-[11px] text-[#71f6b5]">03</span>
      ...
      <div className="border-t border-[#20362f]">
        {experience.map(([date, role, company, copy, current]) => (
          <article
            ...
            key={role}
          >
            <p className="font-mono text-[11px] text-[#71f6b5]">{date}</p>
            <div>
              <h3 className="mt-0 mb-[7px] text-2xl font-medium tracking-[-.03em]">{role}</h3>
              <span className="text-[13px] text-[#bdd0c7]">{company}</span>
              <p className="max-w-[640px] leading-[1.6] text-[#8fa29a]">{copy}</p>
            </div>
            {current && (
              <b className="h-max rounded-full border border-[#335346] px-[9px] py-1.5 font-mono text-[9px] text-[#71f6b5] uppercase max-[650px]:hidden">
                Current
              </b>
            )}
          </article>
