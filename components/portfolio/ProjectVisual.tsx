...
export const ProjectVisual = ({ index }: { index: string }) => {
  if (index === '01')
    return (
      <div className="relative min-h-[520px] overflow-hidden bg-[radial-gradient(circle_at_55%_48%,#7cb8ff21,transparent_40%),linear-gradient(135deg,#0a1714,#112820)] max-phone:min-h-[430px]">
        {homeServoraVisualCards.map(([label, value, copy, pos]) => (
          <div
            key={label}
            className={`absolute flex w-[230px] flex-col rounded-[15px] border border-[#7cb8ff3b] bg-[#0d1e19ed] p-[18px] shadow-[0_30px_60px_#0005] max-phone:w-[170px] ${pos}`}
          >
  ...
  if (index === '02')
    return (
      <div className="relative min-h-[520px] overflow-hidden bg-[#e5e9df] text-[#07110f] max-phone:min-h-[430px]">
        ...
        <div className="absolute top-[10%] left-[20%] size-[410px] rounded-full border border-[#087a4b73] shadow-[inset_0_0_80px_#71f6b53d] max-phone:left-[-8%]" />
        <div className="absolute top-[29%] left-[37%] size-[210px] rounded-full border-[30px] border-[#087a4b1c]" />
        <span className="absolute top-[48%] left-1/2 -translate-x-1/2 -translate-y-1/2 font-mono text-[clamp(3rem,5vw,6rem)] leading-none font-medium tracking-[-.09em] max-phone:text-[49px]">
          100,000
          <small className="mt-3 block text-center font-mono text-[10px] tracking-[.2em] text-[#3a5a4c]">
            LIVE POINTS
          </small>
        </span>
        <div className="absolute right-6 bottom-6 bg-page px-3 py-2.5 font-mono text-[10px] text-accent-bright">
          15+ DOMAIN LAYERS
        </div>
      </div>
    );
  return (
    <div className="relative min-h-[520px] overflow-hidden bg-[linear-gradient(#ffb86b0a_1px,transparent_1px),linear-gradient(90deg,#ffb86b0a_1px,transparent_1px),#1a1711] bg-[length:26px_26px] max-phone:min-h-[430px]">
      <svg
